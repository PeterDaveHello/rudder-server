...
func generateLoadFiles(ctx context.Context, job *UploadJob) (err error) {
	generateAll := job.hasSchemaChanged || slices.Contains(warehousesToAlwaysRegenerateAllLoadFilesOnResume, job.warehouse.Type) || job.config.alwaysRegenerateAllLoadFiles
